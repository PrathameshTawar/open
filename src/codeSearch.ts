...
function isTextLikely(content: string): boolean {
...
export async function searchCode(
  ...
  | { ok: true; results: Array<{ filePath: string; line: number; snippet: string }> }
...
> {
  ...
  try {
    ...
    const found: Array<{ filePath: string; line: number; snippet: string }> = [];
    ...
    for (const root of roots) {
      ...
      for (const absFile of globbed) {
        ...
        if (!read.ok) continue;
        if (!isTextLikely(read.content)) continue;
